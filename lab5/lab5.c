...
/*
    ...
    */
    ...
    //TODO READ XPM AND DO WHAT IS NECESSARY
...
int(video_test_move)(xpm_map_t xpm, uint16_t xi, uint16_t yi, uint16_t xf, uint16_t yf,
    ...
    uint16_t mode=0x105
    vbe_get_mode_info(mode, *mem_info);
    if (vg_set_mode(mode))
        return 1;
    uint8_t Xdisplace,Ydisplace;
    ...
    if (keyboard_subscribe_int(&bit_no2)) {
        printf("Unable to subscribe interruption!");
        return 1;
    }
    bit_no2 = BIT(1);
        ...
        if (is_ipc_notify(ipc_status)) { // received notification 
            switch (_ENDPOINT_P(msg.m_source)) {
                case HARDWARE: // hardware interrupt notification 
                    ...
                    if (msg.m_notify.interrupts &bit_no2) {  //if it gets an interruption, must read the scancode from the OUT_BUF
                        timer_int_handler();
                        framesCounter++;
                        if(framesCounter == ticksPerFrame and speed<0) { //Do: Update frame
                            framesCounter = 0;
                            if (Xdisplace!=0){
                                xi+=Xdisplace;
                            }else{
